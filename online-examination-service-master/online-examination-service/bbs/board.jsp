...
<body>
	<div id="wrapper">
		...
		<%@include file=".././dao/selectBoard.jsp"%>
		<div id="container">
			<div id="board">
				<%
					int p = 1;
				...
				<div id="board_container">
					...
					<div class="row">
						<div class="col-lg-12"
							onclick="b_hit_up('<%=article.getAt_id() %>', '<%=board%>', '<%=curr_page_num %>')"
						...
						<% if (p==1) { %>
							<% if(p!=page_num) { %>
								<button class="btn btn-white btn-height-34 btn-navigate-board" style="width: 120px;"
									onclick="location.href ='./board.jsp?p=<%=p+1%>'">다음 페이지</button>
							<% } %>
						<% } %>
						<% if(p==page_num) { %>
							   <% if(page_num!=1) { %>
							   		<button class="btn btn-white btn-height-34 btn-navigate-board" style="width: 120px;"
							   		onclick="location.href ='./board.jsp?p=<%=p-1%>'">이전 페이지</button>
							   <% } %>
						<% } %>
