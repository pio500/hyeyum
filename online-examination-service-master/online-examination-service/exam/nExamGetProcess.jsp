...
<%
	...
	try {
		...
		Query = "select * from meta_examination WHERE end_time >= DATE_FORMAT('" + date1 + "','%Y-%m-%d %H:%i:%s') and start_time < DATE_FORMAT('" + date1 + "','%Y-%m-%d %H:%i:%s')";
		...
		while (rs.next()) {
			...
			e.overall_score = rs.getInt("score");
		...
		for(int i=0; i<vec.size(); i++) {
			Query ="SELECT * FROM meta_examination_pass where user_id = '" + user_id +"' and examination_no = " + vec.elementAt(i).no;
			pstmt = conn.prepareStatement(Query);
			rs = pstmt.executeQuery(Query);

			while (rs.next()) {
				vec.elementAt(i).score = rs.getInt("examination_score");
			}
			rs.close();
			
			if(vec.elementAt(i).score ==-1) vec.elementAt(i).state = "true";
